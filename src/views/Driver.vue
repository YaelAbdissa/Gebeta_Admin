<template>
    ...
    <transition name="slide" appear>
        <div class="modal" v-if="showModal">
            ...
            <p>Deleting Driver</p>
            <div class="buttons">
                <button @click="deleteDriver">Yes</button>
                <button @click="showModal = false">No</button>
            </div>
        </div>
    </transition>
    <List @delete-item="del" :listItems="drivers"/>
...
export default{
    ...
    methods: {
        async deleteDriver(){
            const res =  await fetch(`api/driver/${this.delId}`, {
                method: 'DELETE'
            })
            const data = await res.json()
            if(res.status === 200){
                this.drivers = this.drivers.filter((driver) => driver._id !== this.delId)
                this.addNotification({id: this.delId, message: data.message,type: 'success'})
            } else{
                this.addNotification({id: 7,message: 'Error deleting driver',type: 'success'})
            }
