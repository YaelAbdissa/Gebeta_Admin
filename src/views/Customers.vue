<template>
    ...
    <List @delete-item="del" :listItems="clients"/>
...
export default{
    ...
    methods: {
        async deleteClient(){
            const res =  await fetch(`api/client/${this.delId}`, {
                method: 'DELETE'
            })
            const data = await res.json()
            if(res.status === 200){
                this.clients = this.clients.filter((client) => client._id !== this.delId)
                this.addNotification({id: this.delId, message: data.message,type: 'success'})
            } else{
                this.addNotification({id: 7,message: 'Error deleting client',type: 'success'})
            }
